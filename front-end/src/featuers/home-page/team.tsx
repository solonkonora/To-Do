...
const getAccountUrl = (username: string, image: boolean = false) => `https://github.com/${username}${image ? ".png" : ""}`;
        ...
        <div className="w-1/2 grid grid-cols-2 gap-4">
          {teamsAvatar.map(({ githubUsername }, index) => (
            index !== currentImg && (
              ...
              >
                <Image src={getAccountUrl(githubUsername, true)} alt="My Avatar" width={500} height={500} />
                <Link href={getAccountUrl(githubUsername)} target="_blank" className="text-secondary-color cursor-pointer font-bold hover:bg-sky-700 p-3">
                  {githubUsername}
                </Link>
              </div>
            )
